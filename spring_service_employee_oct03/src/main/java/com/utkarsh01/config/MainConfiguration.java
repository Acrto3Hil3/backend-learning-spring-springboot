...
public class MainConfiguration {
	...
	public UserDetailsService userDetailsService(PasswordEncoder encoder) {
		UserDetails admin = User.builder().username(firstUserName).password(encoder.encode(firstUserPass))
				.roles(firstUserRole).build();
		UserDetails user = User.builder().username(secondUserName).password(encoder.encode(secondUserPass))
				.roles(secondUserRole).build();

		UserDetails tester = User.builder().username(thirdUserName).password(encoder.encode(thirdUserPass))
